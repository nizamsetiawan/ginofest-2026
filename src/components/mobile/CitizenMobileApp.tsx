...
export const CitizenMobileApp: React.FC = () => {
  ...
  const GOOGLE_ACCOUNTS = [
    {
      name: "Siti Aminah",
      email: "siti.aminah.gresik@gmail.com",
      avatar: "SA",
    ...
    {
      name: "Budi Santoso",
      email: "budi.santoso92@gmail.com",
      avatar: "BS",
    ...
    {
      name: "Nurul Hidayah",
      email: "nurul.hidayah.gk@gmail.com",
      avatar: "NH",
  ...
  // Handle Google Login: Buka dialog Pilih Akun Google
  ...
  // Handle Pilih Akun Google Terpilih
  const handleSelectGoogleAccount = async (account: { name: string; email: string; district: string }) => {
    setIsGoogleAuthenticating(true);
    const defaultDistrict = account.district || "Gresik";

    // Simulasi OAuth 2.0 handshake
    await new Promise((resolve) => setTimeout(resolve, 600));

    const res = await loginCitizenFromFirestore(account.email, "google_oauth_pass", defaultDistrict);
      ...
      <div className="w-full sm:max-w-[380px] min-h-[100dvh] sm:min-h-[760px] sm:max-h-[800px] bg-white sm:rounded-[40px] sm:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.7)] border-0 sm:border-[8px] sm:border-slate-800 flex flex-col relative overflow-hidden">
          ...
          <div className="flex-1 bg-white flex flex-col px-5 py-3 overflow-y-auto animate-in fade-in duration-200">
            ...
            <div>
              <button
                ...
                className="w-full py-2.5 px-4 rounded-xl bg-white hover:bg-slate-50 border border-[#cbd5e1] text-[#071e49] text-[12px] font-bold transition-all shadow-2xs flex items-center justify-center gap-2 cursor-pointer"
              >
                {/* Google G Icon */}
        ...
        {currentScreen === "main" && (
          <div className="flex-1 flex flex-col bg-[#f8fafc] overflow-hidden">
            {/* Top Bar Header */}
            <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 px-3.5 py-2.5 flex items-center justify-between shadow-2xs">
              <div className="flex items-center gap-2">
                <img src="/logo_app.svg" alt="Kcal" className="w-7 h-7 rounded-lg shadow-2xs" />
                <div>
                  <h3 className="text-[12px] font-black text-[#071e49] leading-tight">
            ...
            {/* Bottom Navigation Bar & Native Home Indicator */}
            <div className="absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 z-50 shadow-lg">
              <nav className="px-2 py-1 flex items-center justify-around">
                {[
        ...
        {showGoogleAccountModal && (
          <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-xs flex items-end sm:items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl p-5 max-w-[340px] w-full space-y-3.5 shadow-2xl border border-slate-200 animate-in slide-in-from-bottom-6 duration-300 text-left">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[9.5px] text-slate-500">
                      untuk lanjut ke <strong>Kcal Pantau Gizi</strong>
                    </p>
                  </div>
                </div>
                <button
              ...
              {isGoogleAuthenticating ? (
                <div className="py-7 flex flex-col items-center justify-center space-y-2 text-center">
                  <div className="w-7 h-7 border-3 border-[#1a73e8]/30 border-t-[#1a73e8] rounded-full animate-spin"></div>
                  <p className="text-[11.5px] font-bold text-[#071e49]">Memverifikasi Akun Google...</p>
                  <p className="text-[10px] text-slate-500">Sinkronisasi profil & domisili warga</p>
                </div>
              ) : (
                <>
                  {/* Account List */}
                  <div className="space-y-1 divide-y divide-slate-100 max-h-[240px] overflow-y-auto">
                    {GOOGLE_ACCOUNTS.map((acc, idx) => (
                      <button
                        key={idx}
                        type="button"
                        onClick={() => handleSelectGoogleAccount(acc)}
                        className="w-full pt-2 pb-2 px-2 rounded-2xl hover:bg-blue-50/70 text-left flex items-center justify-between gap-2.5 transition-colors cursor-pointer group"
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className={`w-8 h-8 rounded-full ${acc.avatarBg} text-white font-bold text-[11px] flex items-center justify-center shadow-xs shrink-0`}>
                            {acc.avatar}
                          </div>
                          <div className="min-w-0">
                            <p className="text-[11.5px] font-bold text-[#071e49] group-hover:text-[#1a73e8] transition-colors truncate">
                              {acc.name}
                            </p>
                    ...
                    {/* Custom / Add another account option */}
                    {!showCustomGoogleInput ? (
                      <button
                        type="button"
                        onClick={() => setShowCustomGoogleInput(true)}
                        className="w-full pt-2.5 pb-2 px-2 rounded-2xl hover:bg-slate-50 text-left flex items-center gap-2.5 text-slate-700 transition-colors cursor-pointer"
                      >
                        <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-500 font-bold text-[13px] flex items-center justify-center shrink-0 border border-slate-200">
                          +
                        </div>
                        <div>
                          <p className="text-[11.5px] font-bold text-[#071e49]">
                            Gunakan akun lain
                          </p>
                          <p className="text-[10px] text-slate-500">
                      ...
                      <div className="pt-2.5 space-y-1.5">
                        <input
                          ...
                          className="w-full px-3 py-1.5 rounded-xl bg-slate-50 border border-slate-300 text-[11px] font-medium text-[#071e49] focus:outline-none focus:border-[#1a73e8]"
                        />
                        <input
                          type="email"
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            handleSelectGoogleAccount({
                              name: googleCustomName.trim() || "Warga Gresik",
                              email: googleCustomEmail.trim(),
                              district: loginDistrict || "Gresik",
                            });
                          }}
                          className="w-full py-2 bg-[#1a73e8] hover:bg-[#155fc0] text-white rounded-xl text-[11px] font-bold shadow-xs cursor-pointer"
                        >
                          Lanjutkan Masuk
                        </button>
                      </div>
                  ...
                  <div className="pt-2 border-t border-slate-100 text-[9px] text-slate-400 leading-relaxed">
                    Untuk melanjutkan, Google akan membagikan nama, email, dan profil ke Kcal Pantau Gizi Gresik.
                  </div>
                </>
              )}
            </div>
