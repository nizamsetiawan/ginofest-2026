...
async function saveDocWithUUID(collectionName, data) {
  const uuid = generateUUID();
  const endpoint = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/${collectionName}/${uuid}?key=${apiKey}`;
  const payload = { fields: toFirestoreFields(data, uuid) };
...
async function runSeed() {
  console.log("🔥 Mulai Seeding Firestore dengan ID UUID Unik Standar...");

  console.log("➡️ Seeding master_komoditas (UUID)...");
  for (const c of commodities) {
    await saveDocWithUUID("master_komoditas", c);
  }

  console.log("➡️ Seeding master_harga_pasar (UUID)...");
  for (const p of prices) {
    await saveDocWithUUID("master_harga_pasar", p);
  }

  console.log("➡️ Seeding master_menu_makanan (UUID)...");
  for (const r of recipes) {
    await saveDocWithUUID("master_menu_makanan", r);
  }

  console.log("➡️ Seeding master_nilai_gizi (UUID)...");
  for (const n of nutrition) {
    await saveDocWithUUID("master_nilai_gizi", n);
  }

  console.log("✅ SELESAI! Seluruh data kini menggunakan ID UUID Unik!");
}
