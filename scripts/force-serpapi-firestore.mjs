...
async function getSerpApiGoogleImage(title) {
  try {
    const searchQuery = encodeURIComponent(title + " gizi nutrisi");
    const searchUrl = `https://serpapi.com/search.json?engine=google_images&q=${searchQuery}&gl=id&hl=id&api_key=${serpApiKey}`;
    console.log(`🔎 Searching SerpAPI for: "${title}"...`);
    const res = await fetch(searchUrl);
    if (res.ok) {
      const data = await res.json();
      const images = data.images_results || [];
      if (images.length > 0) {
        // Use thumbnail or original from SerpAPI Google Images
        const serpUrl = images[0].thumbnail || images[0].original || "";
        console.log(`-> Got SerpAPI URL: ${serpUrl}`);
        return serpUrl;
      }
    } else {
...
async function forceSerpApiToFirestore() {
  ...
  for (let i = 0; i < docs.length; i++) {
    ...
    if (title) {
      const serpUrl = await getSerpApiGoogleImage(title);
      if (serpUrl) {
        console.log(`Saving SerpAPI URL to Firestore doc ${docName.split("/").pop()}...`);
        ...
        if (patchRes.ok) {
          updatedCount++;
          console.log(`-> Updated doc ${i + 1}/${docs.length} with SerpAPI image!`);
        } else {
  ...
  console.log(`\n✅ Finished updating ${updatedCount}/${docs.length} articles with SerpAPI Google Images in Firestore!`);
}

forceSerpApiToFirestore().catch(console.error);
