...
async function getAllDocs() {
  const endpoint = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/gscan_help_qa?pageSize=300&key=${apiKey}`;
...
async function deleteDocByUrl(nameUrl) {
  const endpoint = `https://firestore.googleapis.com/v1/${nameUrl}?key=${apiKey}`;
...
async function main() {
  console.log("1. Fetching all existing documents in gscan_help_qa to remove duplicates & old formats...");
  const existingDocs = await getAllDocs();
  console.log(`Found ${existingDocs.length} existing documents.`);

  for (const doc of existingDocs) {
    await deleteDocByUrl(doc.name);
    console.log(`[DELETED] ${doc.name.split("/").pop()}`);
  }

  console.log(`\n2. Seeding ${HELP_QA_DATA.length} cleanly deduplicated unique commands...`);
  let count = 0;
  for (const item of HELP_QA_DATA) {
    ...
    if (status === 200) {
      count++;
      console.log(`[OK] ${item.command} -> ${docId}`);
    }
  }

  console.log(`\nCOMPLETE! Successfully cleaned and synced exactly ${count} unique commands.`);
}
